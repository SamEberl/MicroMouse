...
void compareDistanceToWall(SensorEst& sensor, CellEst labyrinth[LABYRINTH_WIDTH][LABYRINTH_HEIGHT], CornerEst corners[LABYRINTH_WIDTH+1][LABYRINTH_HEIGHT+1]) {
    ...
    if (sensor.looking_at != '0') {
        labyrinth[int(cell_index.x)][int(cell_index.y)].update_wall(sensor.looking_at, true);
        if (sensor.looking_at == 'N') {
            if ((cell_index.y-1) >= 0) {
                labyrinth[int(cell_index.y-1)][int(cell_index.x)].update_wall('S', true);
            }
        }
        if (sensor.looking_at == 'E') {
            if ((cell_index.x+1) < LABYRINTH_WIDTH) {
                labyrinth[int(cell_index.y)][int(cell_index.x+1)].update_wall('W', true);
            }
        }
        if (sensor.looking_at == 'S') {
            if ((cell_index.y+1) < LABYRINTH_HEIGHT) {
                labyrinth[int(cell_index.y+1)][int(cell_index.x)].update_wall('N', true);
            }
        }
        if (sensor.looking_at == 'W') {
            if ((cell_index.x-1) >= 0) {
                labyrinth[int(cell_index.y)][int(cell_index.x-1)].update_wall('E', true);
            }
        }
    }
}
...
void localization(RobotEst* rob){
    ...
    if ((rob->sensL.looking_at == rob->sensL2.looking_at) && rob->sensL.looking_at != '0') {
        ...
        if (driving_dir == 'N' && rob->sensL.looking_at == 'W'){
            num_updates += 1;
            temp_rob_pos.x += rob->sensL.intersection[0] + height;
            ...
            temp_rob_pos.y += rob->sensL.intersection[1] - height;
            temp_rob_dir += angle_rel + M_PI;
    ...
    if ((rob->sensL2.looking_at == rob->sensS.looking_at) && rob->sensL2.looking_at != '0') {
        ...
        if (driving_dir == 'N' && rob->sensS.looking_at=='N'){
            num_updates += 1;
            temp_rob_pos.x += rob->rob_pos.x;
            temp_rob_pos.y += rob->sensS.intersection[1] + height;
            temp_rob_dir += angle_rel + M_PI;
        }
            ...
            num_updates += 1;
            temp_rob_pos.x += rob->sensS.intersection[0] + height;
            temp_rob_pos.y += rob->rob_pos.y;
            temp_rob_dir += angle_rel + M_PI;
        }
    ...
    if ((rob->sensR2.looking_at == rob->sensR.looking_at) && rob->sensR2.looking_at != '0'){
        ...
        if (driving_dir == 'N' && rob->sensR.looking_at=='E'){
            num_updates += 1;
            temp_rob_pos.x += rob->sensR2.intersection[0] - height;
            temp_rob_pos.y += rob->rob_pos.y;
            temp_rob_dir += angle_rel + M_PI;
            ...
            num_updates += 1;
            temp_rob_pos.x += rob->rob_pos.x;
            temp_rob_pos.y += rob->sensR2.intersection[1] + height;
            temp_rob_dir += angle_rel + M_PI;
    ...
    if (rob->sensS.looking_at != '0'){
        if (rob->sensS.looking_at == 'N') {
            ...
            rob->rob_pos.y = (1-L_SMOOTHING)*rob->rob_pos.y + L_SMOOTHING*(rob->sensS.intersection[1] + distance);
        } else if (rob->sensS.looking_at == 'E') {
            distance = (rob->sensS.dist_measure + rob->sensS.offset_position) * cos(rob->sensS.sens_dir);
            rob->rob_pos.x = (1-L_SMOOTHING)*rob->rob_pos.x + L_SMOOTHING*(rob->sensS.intersection[0] - distance);
    ...
    if (rob->sensL2.looking_at != '0'){
        if (rob->sensL2.looking_at == 'N') {
            ...
            rob->rob_pos.y = (1-L_SMOOTHING)*rob->rob_pos.y + L_SMOOTHING*(rob->sensL2.intersection[1] + distance);
        } else if (rob->sensL2.looking_at == 'E') {
            distance = (rob->sensL2.dist_measure + rob->sensL2.offset_position) * cos(rob->sensL2.sens_dir);
            rob->rob_pos.x = (1-L_SMOOTHING)*rob->rob_pos.x + L_SMOOTHING*(rob->sensL2.intersection[0] - distance);
    ...
    if (rob->sensR2.looking_at != '0'){
        if (rob->sensR2.looking_at == 'N') {
            ...
            rob->rob_pos.y = (1-L_SMOOTHING)*rob->rob_pos.y + L_SMOOTHING*(rob->sensR2.intersection[1] + distance);
        } else if (rob->sensR2.looking_at == 'E') {
            distance = (rob->sensR2.dist_measure + rob->sensR2.offset_position) * cos(rob->sensR2.sens_dir);
            rob->rob_pos.x = (1-L_SMOOTHING)*rob->rob_pos.x + L_SMOOTHING*(rob->sensR2.intersection[0] - distance); 
...
void RobotEst::updatePosition(SDL_Renderer *renderer, Robot mouse, CellEst labyrinth[LABYRINTH_WIDTH][LABYRINTH_HEIGHT], CornerEst corners[LABYRINTH_WIDTH+1][LABYRINTH_HEIGHT+1]) {
    ...
    sensR.updatePosition(rob_pos, rob->rob_dir);
    sensR2.updatePosition(rob_pos, rob->rob_dir);
    sensS.updatePosition(rob_pos, rob->rob_dir);
    sensL2.updatePosition(rob_pos, rob->rob_dir);
    sensL.updatePosition(rob_pos, rob->rob_dir);
}